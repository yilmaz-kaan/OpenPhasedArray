...
TURNTABLE_IP          = "172.16.1.186"   # IP address of the FCU3 controller
...
# ── Sweep Parameters ──────────────────────────────────────────────────────────
SWEEP_START_DEG    = -120.0   # Start angle (°); typical values: -180, -90, 0
SWEEP_STOP_DEG     =  60.0   # Stop angle  (°); typical values:  180,  90, 360
SWEEP_STEP_DEG     =    2.0   # Angular step between measurement points (°)
                               # Smaller = finer resolution, more measurement time

HOLD_TIME_SEC      =   2.0    # Max Hold dwell time at each angle (seconds)
...
class SpectrumAnalyzer:
    ...
    def connect(self) -> None:
        ...
        # ── Flush any stale data left in the VISA buffer ───────────────────────
        # A hard power cycle can leave the SA mid-response on the USB/VISA layer.
        # clear() sends USBTMC CLEAR and discards any unread bytes so the
        # subsequent query/write stream starts from a known clean state.
        self._inst.clear()
        self._inst.write("*CLS")          # clear SA status registers & error queue

        idn = self._inst.query("*IDN?").strip()
        log.info("SA connected: %s", idn)

        # ── Select spectrum analyzer mode ──────────────────────────────────────
        # INST:SEL activates the spectrum subsystem.
        self._inst.write("INST:SEL SPECT")

        # INST:SYST SPECT,ACT returns a response on some firmware versions —
        # always call it as query() so the response is consumed and never left
        # sitting in the buffer to corrupt the next command.
        resp = self._inst.query("INST:SYST SPECT,ACT").strip()
        log.debug("INST:SYST SPECT,ACT response: %r", resp)

        # Wait for the mode switch to fully commit before sending subsystem
        # commands — without this barrier "TRAC:STOR:MODE MAXH" can arrive
        # before the spectrum subsystem is ready and return "Insufficient data".
        self._inst.query("*OPC?")

        # ── Configure trace, sweep, and marker ────────────────────────────────
        self._inst.write("TRAC:STOR:MODE MAXH")
        self._inst.write("INITiate:CONTinuous ON")
        self._inst.query("*OPC?")         # wait for first sweep to complete
