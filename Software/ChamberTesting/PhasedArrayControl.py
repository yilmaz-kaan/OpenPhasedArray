...
def reverse_bits(byte_val):
    ...
    Required because RFSA3713 Attenuator expects LSB First, but busio.SPI sends MSB First.
    """
...
def send_spi_command(device_type, device_id, value, decoder_pins):
    ...
    :param value: Integer value to write (0-255)
        Phase Shifter is 6 bit so only 0-63
        Attenuator is 7 bit, so only 0-127
