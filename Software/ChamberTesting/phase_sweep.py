...
SA_SPAN_HZ         = 100e3      # Hz  –  span around center frequency
...
class SpectrumAnalyzer:
    ...
    def connect(self) -> None:
        ...
        log.info("Connecting to spectrum analyzer: %s", self._resource)
        ...
        self._inst.write(f"FREQuency:CENTer {self._center}")
        self._inst.write(f"FREQuency:SPAN {self._span}")

        # Enable Max Hold trace and a normal marker
        self._inst.write("TRACe1:STORage:MODE OFF")
        self._inst.write("INITiate:CONTinuous ON")
        self._inst.write("CALC:MARK:STAT ON")
        self._inst.write("CALC:MARK:MODE NORM")
        log.info("SA enabled, continuous sweep running.")
    ...
    def enable_linear_average(self) -> None:
        ...
        averaging; :SENSe:AVERage:COUNt sets how many sweeps are averaged.
        Continuous sweep must remain ON so averaging runs automatically.
        """
        self._inst.write("TRACe1:STORage:MODE LAV")
        self._inst.write("INITiate:CONTinuous ON")
        log.info("SA switched to Linear Average (LAV), count")
...
def run_phase_sweep(
    ...
    n_values         List of n values to sweep (default: range(64)).
    m_values         List of m values to sweep (default: range(64)).

    Returns a list of all PhaseMeasurements collected (may be partial on
    early exit via KeyboardInterrupt).
    """
    if n_values is None:
        n_values = list(range(64))
    if m_values is None:
        m_values = list(range(64))
...
# ═══════════════════════════════════════════════════════════════════════════════
# CLI  –  sequence parsing helpers
# ═══════════════════════════════════════════════════════════════════════════════

def parse_int_sequence(expr: str, param_name: str = "value") -> List[int]:
    """
    Parse a CLI string into a sorted, deduplicated list of integers in [0, 63].

    Accepted forms (mix and match, comma-separated):
      "range(0,64,4)"     → every 4th value from 0 to 63
      "range(64)"         → 0..63
      "[0,8,16,32]"       → explicit list literal
      "0,8,16,32"         → bare comma-separated integers
      "range(0,32)+[60]"  → range union with extra values (use '+' as separator)

    The expression is evaluated with only Python builtins ``range`` and
    ``list`` in scope — no arbitrary code execution.

    Examples
    ────────
      --n-values "range(0,64,8)"       →  [0, 8, 16, 24, 32, 40, 48, 56]
      --m-values "[0,1,2,3]"           →  [0, 1, 2, 3]
      --n-values "0,16,32,48"          →  [0, 16, 32, 48]
      --m-values "range(0,32)+[60,63]" →  [0..31, 60, 63]
    """
    # Replace '+' join operator with ',' so range(...)+[...] works naturally
    # after we wrap everything in list(set(...)) below.
    cleaned = expr.strip()

    # Collect all integers produced by the expression
    try:
        # Allow list/range literals and bare integers; forbid everything else.
        safe_globals = {"__builtins__": {}, "range": range, "list": list}

        # If the expression looks like bare "a,b,c" integers (no brackets/parens)
        # wrap it as a list literal for eval.
        if cleaned and cleaned[0].isdigit() and "range" not in cleaned and "[" not in cleaned:
            cleaned = f"[{cleaned}]"

        # Handle "range(...)+[...]" style unions by splitting on '+' that sit
        # outside parentheses/brackets, then evaluating each piece separately.
        pieces  = _split_top_level_plus(cleaned)
        values: List[int] = []
        for piece in pieces:
            result = eval(piece, safe_globals)   # noqa: S307
            # Accept range, list, or bare int
            if isinstance(result, int):
                values.append(result)
            else:
                values.extend(int(v) for v in result)

    except Exception as exc:
        raise argparse.ArgumentTypeError(
            f"Could not parse --{param_name} expression {expr!r}: {exc}\n"
            "  Valid examples: \"range(0,64,8)\", \"[0,8,16]\", \"0,16,32,48\", "
            "\"range(0,32)+[60,63]\""
        ) from exc

    # Validate range
    bad = [v for v in values if not (0 <= v <= 63)]
    if bad:
        raise argparse.ArgumentTypeError(
            f"--{param_name}: values must be in [0, 63]; got out-of-range: {bad}"
        )
...
def _split_top_level_plus(expr: str) -> List[str]:
    """
    Split *expr* on '+' characters that are not inside brackets or parentheses.
    e.g. "range(0,32)+[60,63]" → ["range(0,32)", "[60,63]"]
    """
    pieces  = []
    depth   = 0
    current: List[str] = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep arithmetic phase-vector combinations and record "
            "SA peak power for each state.\n\n"
            "By default all 4096 combinations (n,m ∈ 0..63) are measured. "
            "Use --n-values / --m-values to restrict the sweep to a subset."
        ),
    ...
    parser.add_argument(
        ...
        metavar="EXPR",
        help=(
            "Python expression defining which n values to sweep. "
            "Accepts range(), list literals, bare integers, or unions with '+'. "
            "All values must be in [0, 63]. "
            "Default: range(64)  (all 64 values). "
            "Examples:\n"
            "  --n-values \"range(0,64,8)\"       → every 8th value\n"
            "  --n-values \"[0,16,32,48]\"         → explicit list\n"
            "  --n-values \"0,16,32,48\"           → bare integers\n"
            "  --n-values \"range(0,32)+[60,63]\"  → range union\n"
        ),
    )
    parser.add_argument(
        "--m-values",
        ...
        metavar="EXPR",
        help=(
            "Python expression defining which m values to sweep. "
            "Same syntax as --n-values. Default: range(64)."
        ),
    )
...
def main() -> None:
    ...
    # ── Resolve n / m value lists ─────────────────────────────────────────────
    n_values: List[int] = (
        parse_int_sequence(args.n_values, param_name="n-values")
        if args.n_values is not None
        else list(range(64))
    )
    m_values: List[int] = (
        parse_int_sequence(args.m_values, param_name="m-values")
        if args.m_values is not None
        else list(range(64))
    )

    total_states = len(n_values) * len(m_values)
    ...
    log.info("  n values    : %d  %s", len(n_values), n_values if len(n_values) <= 16 else f"[{n_values[0]}..{n_values[-1]}]")
    log.info("  m values    : %d  %s", len(m_values), m_values if len(m_values) <= 16 else f"[{m_values[0]}..{m_values[-1]}]")
    log.info("  Total states: %d", total_states)
    log.info("  Est. runtime: ~%.0f min", total_states * (args.dwell + args.settle) / 60)
    log.info("=" * 60)
