...
SA_SETTLE_SEC      = 0.15      # Wait time after re-configuring before querying
...
class PhaseCalibrator:
    ...
    Rounds repeat until no improvement is found or the round limit is hit.
    ...
    def _measure(self, phases: List[int]) -> Measurement:
        """Apply a phase vector and return a Measurement."""
    ...
    def _sweep_one_shifter(
        ...
        step size.  Return (updated_phases, best_power_dbm).
        """
        states = range(0, PHASE_STATES, step)
        ...
        for state in states:
            ...
            if m.power_dbm > best_power:
                best_power  = m.power_dbm
                best_phases = list(candidate)

        log.info(
            "  Shifter %d best state: %2d  (%.2f dBm)",
    ...
    def _coordinate_descent_phase(
        ...
        """
        Run coordinate descent at a given step size.

        Returns (best_phases, best_power_dbm).
        """
        log.info("═" * 60)
        ...
        # Measure the initial state before iterating
        m0 = self._measure(current_phases)
        best_power = m0.power_dbm

        for round_num in range(1, max_rounds + 1):
            log.info("── Round %d / %d ──────────────────────────────", round_num, max_rounds)
            power_before_round = best_power

            for idx in range(NUM_PHASE_SHIFTERS):
                current_phases, best_power = self._sweep_one_shifter(
                    current_phases, idx, step
                )

            improvement = best_power - power_before_round
            log.info(
                "Round %d complete.  Power: %.2f dBm  (Δ = %+.3f dBm)",
                round_num, best_power, improvement
            )
            log.info("Current phases: %s", current_phases)

            # Convergence: no improvement in this round
            if improvement <= 0.0:
                log.info("%s converged after %d round(s).", phase_label, round_num)
                break

        return current_phases, best_power

    # ── Public API ─────────────────────────────────────────────────────────────

    def run(self) -> Measurement:
        """
        Execute the full two-phase calibration and return the best Measurement.
        """
        t_start = time.monotonic()

        # ── Phase 1: Coarse coordinate descent ────────────────────────────────
        init_phases = [0] * NUM_PHASE_SHIFTERS
        coarse_best_phases, coarse_best_power = self._coordinate_descent_phase(
        ...
        log.info("\nCoarse optimum: %s  →  %.2f dBm", coarse_best_phases, coarse_best_power)

        # ── Phase 2: Fine coordinate descent ──────────────────────────────────
        fine_best_phases, fine_best_power = self._coordinate_descent_phase(
            init_phases   = coarse_best_phases,
            step          = 1,
        ...
        # ── Final measurement at the confirmed optimum ─────────────────────────
        log.info("\nApplying final phase vector for confirmation measurement …")
        final = self._measure(fine_best_phases)

        log.info("═" * 60)
        log.info("CALIBRATION COMPLETE")
        log.info("  Best phase vector : %s", fine_best_phases)
        log.info("  Peak power        : %+.2f dBm", final.power_dbm)
        log.info("  Marker frequency  : %.6f GHz", final.freq_hz / 1e9)
